...
int main() {
	...
	if (escolha==1)
	{
		banana=fopen("moranguinho.txt", "w+"); /*Abre um arquivo no formato .txt com o nome "moranguinho" e escreve nele*/
		if (banana==NULL)
			printf("Não dá pra abrir, pq n existe\n");
		
		printf("Escreva %d frases que serão colocadas no arquivo .txt:\n", quantidade);
		...
		for (i = 0; i < quantidade; ++i)
		{
			fprintf(banana, "%s\n", frase[i]); /*Escreve o q estava no vetor de frases*/
		}
		printf("Agora escreva %d números que serão colocadas no arquivo .txt: \n", quantidade);
		for (i = 0; i < quantidade; ++i)
		{
			scanf("%d", &valores[i]);
			fprintf(banana, "%d\n", valores[i]);
